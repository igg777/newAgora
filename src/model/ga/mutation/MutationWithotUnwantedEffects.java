...
public class MutationWithotUnwantedEffects implements IGeneticAlgorithmStep {
	...
	private Problem makeMutation(Problem childToMutate) throws FileNotFoundException {
		...
		// como no se mutan las causas raices entonces no se pueden tener en
		// cuenta en la cant de ind a mutar
		int aux = childToMutate.getCantidadInditators()
				- childToMutate.getUnwantedEffects().size();
		
		System.out.println("Cantidad de indicadores que no son efectos"+aux);
		...
		for (int i = 0; i < aux; i++) {
			...
			indicatorAux = OntologyIndicators.get(random
		...
		System.out.println("Selecionadolos inicadores ");
		// mando a mutar al nuevo problema dandole los indicadores nuevos que lo
		// formaran.

		problemTemp.mutatingByGoodBloqsWEffects(indicatorsSelected);
	...
	public ArrayList<Problem> run(ArrayList<Problem> problems) throws FileNotFoundException {
		...
		for (int i = 0; i < problems.size(); i++) {
			...
			if (probability <= MUTATIONPROBABILITY) {
				System.out.println("Entro a  mutar");  
