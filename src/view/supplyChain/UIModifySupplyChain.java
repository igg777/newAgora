 ...
 */
public class UIModifySupplyChain extends javax.swing.JFrame {
	...
	private String nameOfTheSCToModify;
	private JLabel jlblenterprisesss;
	private JLabel jlblnameee;

	public String getNameOfTheSCToModify() {
		return nameOfTheSCToModify;
	}

	public void setNameOfTheSCToModify(String nameOfTheSCToModify) {
		this.nameOfTheSCToModify = nameOfTheSCToModify;
	}
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				UIModifySupplyChain inst = new UIModifySupplyChain();
	...
	private void initGUI() {
		try {
			...
			this.setTitle("Modify supply chain");
			{
				jbtnAdd = new JButton();
			...
			}
			{
				...
				getContentPane().add(jbtnModify, "13, 33, 15, 34");
			...
			}
			{
				...
				getContentPane().add(jlblnameee, "8, 2, 8, 3");
				jlblnameee.setText("*");
	...
	private void jbtnModifyActionPerformed(ActionEvent evt) {
		try{
		...
		if (enterprises.size() > 0) {
			if (jtfName.getText().equals("") == false
                ...
                SupplyChainController controllerForEnterprises = new SupplyChainController(nameOfTheSCToModify);
                controllerForEnterprises.removeAllEnterprises();
				SupplyChainController controller = new SupplyChainController(
						jtfName.getText(), jtpComments.getText());
				controller.updateSupplyChain(nameOfTheSCToModify);
	...
	/**
	 * fills all the visual components whit the data in DB. 
	 */
	public void loadOldData(){
		try{
			
		SupplyChainController controller = new SupplyChainController(nameOfTheSCToModify);
		DefaultComboBoxModel[] models = controller.getModelsForModifyInterface();
		jlEnterprises.setModel(models[0]);
		jlSCEnterprises.setModel(models[1]);
		jtfName.setText(nameOfTheSCToModify);
		jtpComments.setText(controller.getCommentsFromDB());
		}
		catch (Exception e) {
			e.printStackTrace();
			GraphEd.errorPane.printMessage(e);
